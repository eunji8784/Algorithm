...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		cloudLocs.add(new Location(n - 1,  0));
		cloudLocs.add(new Location(n - 1,  1));
		cloudLocs.add(new Location(n - 2,  0));
	...
	private static void copyMagic() {
		...
		int size =  cloudLocs.size();
		while (size-- > 0) {
			Location cloudLoc = cloudLocs.poll();
			int count = 0;
			for (int i = 0; i < 4; i++) {
				int nx = cloudLoc.x + DX[DIR[i]];
				int ny = cloudLoc.y + DY[DIR[i]];
				if (nx >= 0 && nx < n && ny >= 0 && ny < n && map[nx][ny] > 0) {
					count++;
				}
			}
			waters.add(new int[] {cloudLoc.x, cloudLoc.y, count});
			cloudLocs.add(cloudLoc);
		}
